    ...
    void SetJointPositions::UpdateChild()
    {
        ...
        for (std::size_t i = 0; i < last_joint_state.name.size(); ++i)
        {
            ...
            else
            {
                ...
                {
                    RCLCPP_WARN_STREAM_THROTTLE(nh_->get_logger(), *nh_->get_clock(), 1000, "Joint " << (*it)->GetName() << " is above upper limit " << position << " > " << upper_limit);
                    position = upper_limit;
                }
                else if (position < lower_limit)
                {
                    RCLCPP_WARN_STREAM_THROTTLE(nh_->get_logger(), *nh_->get_clock(), 1000, "Joint " << (*it)->GetName() << " is below lower limit " << position << " < " << lower_limit);
                ...
                for (Iter it_mimic = joints_list_.begin(); it_mimic != joints_list_.end(); ++it_mimic)
                {
                    ...
                    if (set_mimic)
                    {
                        ...
                        if (position > upper_limit)
                        {
                            RCLCPP_WARN_STREAM_THROTTLE(nh_->get_logger(), *nh_->get_clock(), 1000, "Joint " << (*it_mimic)->GetName() << " is above upper limit " << position << " > " << upper_limit);
                            position = upper_limit;
                        }
                        else if (position < lower_limit)
                        {
                            RCLCPP_WARN_STREAM_THROTTLE(nh_->get_logger(), *nh_->get_clock(), 1000, "Joint " << (*it_mimic)->GetName() << " is below lower limit " << position << " < " << lower_limit);
                            position = lower_limit;
                        }

                        RCLCPP_DEBUG_STREAM_THROTTLE(nh_->get_logger(), *nh_->get_clock(), 1000, "Updating joint " << (*it_mimic)->GetName() << " from " << old_angle << " to "
                                                                      << position);
